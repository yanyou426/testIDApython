...
if __name__ == '__main__':
    if not idaapi.get_plugin_options("CMR"):
        print("[!] -OCMR option is missing")
        ida_pro.qexit(1)

    plugin_options = idaapi.get_plugin_options("CMR").split(";")
    if len(plugin_options) != 3:
        print("[!] -OCMR:INPUT_JSON:IDB_PATH:OUTPUT_DIR")
        ida_pro.qexit(1)
